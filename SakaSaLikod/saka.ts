...
// ============ WILDTRACK API ENDPOINTS ============

// Get all species or filter by mountain
...
// Get species for a specific mountain
...
app.get('/api/wildtrack/stats', authenticateToken, (req: any, res: any) => {
  const { mountain_id } = req.query;
  console.log(`[WildTrack] Fetching stats for user ${req.user.id}, mountain ${mountain_id || 'all'}`);
  ...
  db.get(countQuery, countParams, (err: Error | null, countRow: any) => {
    if (err) {
      console.error('[WildTrack] Error fetching discovery count:', err.message);
      return res.status(500).json({ error: 'Failed to fetch stats' });
    }

    console.log(`[WildTrack] Discovered count: ${countRow.discovered_count}`);

    // Get total curated species count from mountain_biodiversity table
    ...
    if (mountain_id) {
      ...
      // If no mountain specified, sum all curated species counts
    ...
    db.get(totalQuery, totalParams, (err: Error | null, totalRow: any) => {
      if (err) {
        console.error('[WildTrack] Error fetching total species count:', err.message);
        return res.status(500).json({ error: 'Failed to fetch stats' });
      }

      const totalSpecies = totalRow.total_species || 0;
      console.log(`[WildTrack] Total curated species: ${totalSpecies}`);
      ...
      db.all(categoryQuery, categoryParams, (err: Error | null, categoryRows: any[]) => {
        if (err) {
          console.error('[WildTrack] Error fetching category stats:', err.message);
          return res.status(500).json({ error: 'Failed to fetch stats' });
        }
        ...
        console.log(`[WildTrack] Progress: ${countRow.discovered_count}/${totalSpecies} (${percentage}%)`);
...
// Get mountain biodiversity info
app.get('/api/wildtrack/mountain-info/:id', (req: any, res: any) => {
  ...
  db.get(
    ...
    (err: Error | null, info: any) => {
      ...
      if (!info) {
        console.error(`[WildTrack] Mountain info not found for ID: ${id}`);
        return res.status(404).json({ error: 'Mountain info not found' });
      }

      console.log(`[WildTrack] Retrieved mountain info for ${info.name}`);
      res.json({ info });
    }
  );
});
...
app.delete('/api/wildtrack/discovery/:id', authenticateToken, (req: any, res: any) => {
  ...
  db.get(
    ...
    (err: Error | null, discovery: any) => {
      if (err) {
        console.error('[WildTrack] Error checking discovery:', err.message);
        return res.status(500).json({ error: 'Database error' });
      }

      if (!discovery) {
        console.error(`[WildTrack] Discovery not found for ID: ${id}, user: ${req.user.id}`);
      ...
      db.run(
        ...
        function(this: any, err: Error | null) {
          if (err) {
            console.error('[WildTrack] Error deleting discovery:', err.message);
            return res.status(500).json({ error: 'Failed to delete discovery' });
          }

          console.log(`[WildTrack] Discovery removed: ID ${id}, user ${req.user.id}`);
...
app.get('/api/wildtrack/location-species', async (req: any, res: any) => {
  ...
  try {
    // Get species from GBIF by location
    ...
    // Get species from iNaturalist by location
