...
export const INaturalistService = {
  async searchTaxa(query: string, perPage: number = 16): Promise<SpeciesSearchResult[]> {
    const data = await safeJsonFetch<{ results?: any[] }>(
      `${BASE_URL}/taxa?q=${encodeURIComponent(query)}&per_page=${perPage}`
    );
    ...
    return results.map((item: any) => ({
      ...
      description: item.wikipedia_summary || [item.family_name, item.genus_name, item.species_name].filter(Boolean).join(' > ') || item.rank || 'Species',
    }));
  },

  async getTaxonDetails(taxonId: number): Promise<SpeciesDetails | null> {
    const data = await safeJsonFetch<{ results?: any[] }>(`${BASE_URL}/taxa/${taxonId}`);
    const taxon = data?.results?.[0];
    if (!taxon) return null;

    const photos = (taxon.taxon_photos || [])
      .map((photo: any) => photo.photo?.medium_url || photo.photo?.original_url)
      .filter(Boolean);
